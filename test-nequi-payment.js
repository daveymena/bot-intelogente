/**
 * Test rápido para verificar que el número de Nequi se envía correctamente
 */
...
async function testNequiPayment() {
  ...
  try {
    ...
    const orchestratorModule = await import('./src/agents/orchestrator.ts');
    const Orchestrator = orchestratorModule.Orchestrator;
    ...
    if (response.actions && response.actions.length > 0) {
      ...
      response.actions.forEach((action, i) => {
        ...
        if (action.formattedText) {
          console.log(`   Texto formateado: ${action.formattedText.substring(0, 200)}`);
