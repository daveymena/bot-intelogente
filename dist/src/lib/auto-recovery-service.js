...
function getGroq() {
    if (!groqInstance && process.env.GROQ_API_KEY) {
        groqInstance = new groq_sdk_1.default({ apiKey: process.env.GROQ_API_KEY });
    }
    if (!groqInstance) {
...
class AutoRecoveryService {
     ...
     */
    static async diagnoseError(component, error) {
        try {
...
Responde en formato JSON:
{
  ...
  "accion": "comando o código específico"
     ...
     */
    static async startMonitoring(intervalMinutes = 5) {
    ...
    static async checkPaymentsHealth() {
        try {
     ...
     */
    static logRecovery(component, error, diagnosis, action, success) {
        const log = {
            timestamp: new Date(),
            component,
     ...
     */
    static getRecoveryLogs() {
        return recoveryLogs;
    }
    /**
     * Recuperación completa del sistema
     */
    static async fullSystemRecovery() {
