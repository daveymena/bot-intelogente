...
class HumanEscalationService {
     ...
     */
    static needsHumanEscalation(message) {
     ...
     */
    static async collectCustomerInfo(customerPhone, customerMessage, category) {
        const questions = [];
     ...
     */
    static async notifyAdmin(userId, customerPhone, customerName, category, customerMessage, collectedInfo) {
        try {
            ...
            const sent = await whatsapp_web_service_1.WhatsAppWebService.sendMessage(userId, this.ADMIN_PHONE + '@s.whatsapp.net', notification);
            if (sent) {
     ...
     */
    static async saveEscalation(userId, customerPhone, category, message) {
        try {
            // Buscar conversación
     ...
     */
    static generateEscalationResponse(category) {
        const responses = {
