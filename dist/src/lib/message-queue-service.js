...
class MessageQueueService {
     ...
     */
    static async enqueue(phoneNumber, message, type = 'text', metadata) {
        try {
            console.log('[Queue] 📬 Agregando mensaje a la cola:', { phoneNumber, type });
            // Guardar en base de datos
            await db_1.db.messageQueue.create({
                data: {
                    phoneNumber,
                    message,
                    type,
     ...
     */
    static async processPendingMessages(sendFunction) {
        ...
        try {
            ...
            const pendingMessages = await db_1.db.messageQueue.findMany({
                where: {
                    status: 'PENDING',
            ...
            for (const msg of pendingMessages) {
                try {
                    ...
                    await db_1.db.messageQueue.update({
                        where: { id: msg.id },
    ...
    static async cleanOldMessages() {
        try {
            ...
            const result = await db_1.db.messageQueue.deleteMany({
                where: {
                    OR: [
    ...
    /**
     * Limpiar cola de un usuario específico
     */
    static async clearUserQueue(userId) {
        try {
            console.log(`[Queue] 🧹 Limpiando cola para usuario ${userId}`);
            // Obtener conexión del usuario para saber su número
            ...
            if (!connection || !connection.phoneNumber || connection.phoneNumber === 'pending') {
                console.log(`[Queue] ⚠️ No se encontró número de teléfono para usuario ${userId}`);
                return 0;
            }
            ...
            console.log(`[Queue] ✅ ${result.count} mensajes eliminados de la cola`);
            return result.count;
        }
        catch (error) {
            console.error('[Queue] ❌ Error limpiando cola de usuario:', error);
            return 0;
        }
    }
}
