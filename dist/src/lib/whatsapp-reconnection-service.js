...
exports.WhatsAppReconnectionService = void 0;
...
class WhatsAppReconnectionService {
     ...
     */
    static async performHealthCheck(userId) {
        try {
            const session = whatsapp_web_service_1.WhatsAppWebService.getSession(userId);
            if (!session) {
     ...
     * 🔄 Intentar reconexión
     */
    static async attemptReconnection(userId) {
        const state = this.reconnectionStates.get(userId);
        if (!state) {
        ...
        const delay = Math.min(this.config.initialDelay * Math.pow(this.config.backoffMultiplier, state.retryCount - 1), this.config.maxDelay);
     ...
     * ⚡ Ejecutar reconexión
     */
    static async executeReconnection(userId) {
        const state = this.reconnectionStates.get(userId);
        if (!state)
            return;
        ...
        try {
            ...
            await whatsapp_web_service_1.WhatsAppWebService.disconnect(userId);
            ...
            const result = await whatsapp_web_service_1.WhatsAppWebService.initializeConnection(userId);
            if (result.success) {
     ...
     */
    static async notifyReconnectionSuccess(userId) {
        try {
     ...
     */
    static async notifyReconnectionFailure(userId) {
        try {
     ...
     */
    static async forceReconnection(userId) {
     ...
     */
    static getReconnectionState(userId) {
...
exports.WhatsAppReconnectionService = WhatsAppReconnectionService;
WhatsAppReconnectionService.config = {
    maxRetries: 10,
...
WhatsAppReconnectionService.reconnectionStates = new Map();
WhatsAppReconnectionService.healthCheckIntervals = new Map();
WhatsAppReconnectionService.reconnectionTimeouts = new Map();
