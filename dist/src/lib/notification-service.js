...
class NotificationService {
    ...
    static async createToken(params) {
        const { type, purpose, userId, paymentId, metadata, expiresInHours = 24 } = params;
        ...
        // Guardar en BD
        const tokenRecord = await db_1.db.notificationToken.create({
            data: {
                token: hashedToken,
                type,
                purpose,
    ...
    static async validateToken(rawToken) {
        const hashedToken = crypto_1.default
            ...
            .update(rawToken)
            .digest('hex');
        const tokenRecord = await db_1.db.notificationToken.findFirst({
            where: {
                token: hashedToken,
                status: 'PENDING',
                expiresAt: {
        ...
        await db_1.db.notificationToken.update({
     ...
     */
    static async sendPaymentConfirmation(userId, paymentId, customerEmail, amount, currency = 'COP', customerName) {
    ...
    static generatePaymentReminderEmail(data) {
        return `
      <!DOCTYPE html>
      <html>
        <head>
