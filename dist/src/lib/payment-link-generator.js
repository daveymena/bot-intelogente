...
exports.PaymentLinkGenerator = void 0;
const db_1 = require("./db");
class PaymentLinkGenerator {
    /**
     * Generar link de MercadoPago
     */
    static async generateMercadoPagoLink(productName, price, productId) {
        try {
     ...
     */
    static async getPayPalAccessToken(clientId, clientSecret) {
        const mode = process.env.PAYPAL_MODE || 'live';
     ...
     */
    static generatePayPalFallbackLink(productName, price) {
        const exchangeRate = parseFloat(process.env.COP_TO_USD_RATE || '4000');
        const priceUSD = (price / exchangeRate).toFixed(2);
     ...
     */
    static async generatePaymentLinks(productId) {
        try {
            ...
            const mercadopagoLink = await this.generateMercadoPagoLink(product.name, product.price, product.id);
            const paypalLink = await this.generatePayPalLink(product.name, product.price, product.id);
            // Construir respuesta
            const paymentLinks = {
                product,
                methods: {
     ...
     */
    static generateInstructions(product, mercadopagoLink, paypalLink) {
    ...
    static getProductEmoji(product) {
        ...
        if (name.includes('moto'))
            return '🏍️';
        if (name.includes('curso') || name.includes('mega'))
            return '📚';
        return '✨';
    }
    /**
     ...
     */
    static formatForWhatsApp(paymentLinks) {
        return paymentLinks.instructions;
    }
    /**
     * Generar respuesta según método elegido
     */
    static generateMethodResponse(method, paymentLinks) {
...
PaymentLinkGenerator.NEQUI_NUMBER = process.env.NEQUI_NUMBER || '3136174267';
PaymentLinkGenerator.DAVIPLATA_NUMBER = process.env.DAVIPLATA_NUMBER || '3136174267';
// Credenciales de MercadoPago (desde .env)
PaymentLinkGenerator.MERCADOPAGO_ACCESS_TOKEN = process.env.MERCADO_PAGO_ACCESS_TOKEN || '';
PaymentLinkGenerator.MERCADOPAGO_PUBLIC_KEY = process.env.MERCADO_PAGO_PUBLIC_KEY || '';
// Configuración de PayPal
PaymentLinkGenerator.PAYPAL_EMAIL = process.env.PAYPAL_EMAIL;
PaymentLinkGenerator.PAYPAL_ME_USERNAME = process.env.PAYPAL_ME_USERNAME || process.env.PAYPAL_USERNAME;
PaymentLinkGenerator.COP_TO_USD_RATE = parseFloat(process.env.COP_TO_USD_RATE || '4000');
