...
exports.BaileysStableService = void 0;
const conversational_module_1 = require("@/conversational-module");
...
const voice_generation_service_1 = require("./voice-generation-service");
const Personality = __importStar(require("./conversational-personality"));
// 🛡️ Sistema Anti-Ban
const safe_baileys_sender_1 = require("./safe-baileys-sender");
const safe_reconnect_manager_1 = require("./safe-reconnect-manager");
// 🤖 Simulador de Escritura Humana
const human_typing_simulator_1 = require("./human-typing-simulator");
class BaileysStableService {
     ...
     */
    static async initializeHybridSystem() {
     ...
     * Configurar manejadores de eventos
     */
    static async setupEventHandlers(socket, session, saveCreds, userId) {
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    ...
                    safe_reconnect_manager_1.SafeReconnectManager.recordDisconnect(userId);
                    session.reconnectAttempts++;
                    // 🛡️ Verificar si puede reconectar (protección anti-ban)
                    if (!safe_reconnect_manager_1.SafeReconnectManager.canReconnect(userId)) {
                    ...
                    const success = await safe_reconnect_manager_1.SafeReconnectManager.startReconnect(userId, async () => {
     ...
     * Configurar manejador de mensajes
     */
    static setupMessageHandler(socket, userId) {
                ...
                try {
                    ...
                    if (message.message?.audioMessage) {
                        ...
                        try {
                            ...
                            const tempPath = path_1.default.join(process.cwd(), 'temp-audio', `audio_${Date.now()}.ogg`);
                            await fs_1.default.promises.mkdir(path_1.default.dirname(tempPath), { recursive: true });
                            ...
                            // Limpiar
                            await fs_1.default.promises.unlink(tempPath).catch(() => { });
                        }
                        catch (error) {
                    ...
                    try {
                        ...
                        const historyMessages = await db_1.db.message.findMany({
                        ...
                        const { AgentOrchestrator } = await Promise.resolve().then(() => __importStar(require('@/agents/agent-orchestrator-wrapper')));
                        ...
                        if (aiResponse.shouldSendPhotos && aiResponse.photos && aiResponse.photos.length > 0) {
                            ...
                            try {
                                const { MediaService } = await Promise.resolve().then(() => __importStar(require('./media-service')));
                                ...
                                const imageData = await MediaService.prepareImageMessage(firstPhoto, aiResponse.message);
                                ...
                                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, aiResponse.message, messageText.length);
                        ...
                        const { handleMessageWithIntelligence } = await Promise.resolve().then(() => __importStar(require('./intelligent-baileys-integration')));
     ...
     */
    static async handleAutoResponse(socket, userId, from, messageText, conversationId) {
        ...
        try {
            ...
            for (let intento = 1; intento <= 3; intento++) {
                try {
                    await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, intelligentResponse.message, messageText.length);
     ...
     */
    static async handleHybridResponse(socket, userId, from, messageText, conversationId) {
        ...
        try {
            ...
            const { NumericSelectionDetector } = await Promise.resolve().then(() => __importStar(require('./numeric-selection-detector')));
            const selection = await NumericSelectionDetector.detectSelection(messageText, history, userId);
            if (selection.isSelection && selection.selectedProduct) {
                ...
                try {
                    const { ProductPhotoSender } = await Promise.resolve().then(() => __importStar(require('./product-photo-sender')));
                    const confirmMessage = `¡Perfecto! 😊 Elegiste la opción ${selection.selectedNumber}\n\nTe envío los detalles:`;
                    await human_typing_simulator_1.HumanTypingSimulator.quickHumanizedSend(socket, from, confirmMessage);
                    await this.saveOutgoingMessage(userId, from, confirmMessage, conversationId);
                    // Enviar producto con foto
                    const result = await ProductPhotoSender.sendProductsWithPhotos(socket, from, [selection.selectedProduct], 1);
                    console.log(`[Baileys] ✅ Producto enviado: ${result.sent}`);
                    // Actualizar historial
                    history.push({ role: 'user', content: messageText }, { role: 'assistant', content: `Producto seleccionado: ${selection.selectedProduct.name}` });
            ...
            if (this.hybridSystem && typeof this.hybridSystem.processMessage === 'function') {
                ...
                const response = await this.hybridSystem.processMessage(messageText, userId, history, from);
                // Enviar respuesta con simulación humana
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, response, messageText.length);
                // Guardar en DB
                await this.saveOutgoingMessage(userId, from, response, conversationId);
                // Actualizar historial
                history.push({ role: 'user', content: messageText }, { role: 'assistant', content: response });
                if (history.length > 20) {
                    history = history.slice(-20);
                }
                this.conversationHistories.set(from, history);
            ...
            const { intelligentProductSearch, generateProductResponse } = await Promise.resolve().then(() => __importStar(require('./intelligent-product-search')));
            ...
            const productMatch = await intelligentProductSearch({
                ...
                previousProducts,
                conversationHistory: history.map((h) => h.content)
            });
            ...
            if (productMatch && productMatch.confidence >= 70) {
                ...
                try {
                    ...
                    if (productMatch.isGeneralQuery && productMatch.products) {
                        ...
                        await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, optionsMessage, messageText.length);
                        ...
                        // Actualizar historial
                        history.push({ role: 'user', content: messageText }, { role: 'assistant', content: optionsMessage });
                    ...
                    if (productMatch.reason && productMatch.reason.length > 50 && products.length === 1) {
                        ...
                        await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, recommendationMessage, messageText.length);
                    ...
                    const result = await ProductPhotoSender.sendProductsWithPhotos(socket, from, products, 5 // máximo 5 productos
                    );
                    ...
                    if (products.length > 0) {
                        ...
                        ConversationContextService.setProductContext(conversationKey, lastProduct.id, lastProduct.name);
                    }
                    // Guardar en DB
                    await this.saveOutgoingMessage(userId, from, `[Enviados ${result.sent} productos con fotos]`, conversationId);
                    // Actualizar historial
                    history.push({ role: 'user', content: messageText }, { role: 'assistant', content: `Productos enviados: ${products.map((p) => p.name).join(', ')}` });
                    if (history.length > 10) {
                        history = history.slice(-10);
                    }
                    this.conversationHistories.set(from, history);
            ...
            if (this.hybridSystem && typeof this.hybridSystem.processMessage === 'function') {
                ...
                response = await this.hybridSystem.processMessage(messageText, userId, history, from);
            }
            else {
                // Fallback: usar sistema local (solo BD)
                console.log('[Baileys] 📦 Procesando con sistema local (solo BD)');
                const { IntelligentProductQuerySystem } = await Promise.resolve().then(() => __importStar(require('./intelligent-product-query-system')));
                response = await IntelligentProductQuerySystem.processQuery(messageText, userId, history);
            }
            ...
            if (process.env.VOICE_ENABLED === 'true') {
                try {
                    const voiceService = new voice_generation_service_1.VoiceGenerationService();
            ...
            await human_typing_simulator_1.HumanTypingSimulator.quickHumanizedSend(socket, from, fallbackResponse);
        }
    }
    /**
     ...
     */
    static async saveOutgoingMessage(userId, to, content, conversationId) {
        let conversation = conversationId
            ? await db_1.db.conversation.findUnique({ where: { id: conversationId } })
            : await db_1.db.conversation.findFirst({ where: { userId, customerPhone: to } });
        if (!conversation) {
     ...
     */
    static async sendMessage(userId, to, content) {
        try {
            const session = this.sessions.get(userId);
            ...
            const success = await safe_baileys_sender_1.SafeBaileysSender.sendText(session.socket, {
                userId,
                recipient: to,
                message: content,
                forceHumanize: true
     ...
     */
    static async sendMessageDirect(userId, to, content) {
        try {
            const session = this.sessions.get(userId);
            if (!session || !session.socket || session.status !== 'CONNECTED') {
                console.error('[Baileys] ❌ No hay sesión activa');
     ...
     * Limpiar sesión
     */
    static async cleanupSession(userId) {
     ...
     */
    static async sendProductPhotosIfRequested(socket, userId, to, messageText, conversationId) {
        try {
            ...
            const { createGroqHybridSystem, HybridIntelligentResponseSystem } = await Promise.resolve().then(() => __importStar(require('./hybrid-intelligent-response-system')));
            const { CustomGreetingSystem } = await Promise.resolve().then(() => __importStar(require('./custom-greeting-system')));
            for (let i = 0; i < Math.min(photos.length, 3); i++) { // Máximo 3 fotos
                ...
                try {
                    ...
                    const imageData = await MediaService.prepareImageMessage(photoUrl, i === 0 ? `${product.name}\n💰 $${product.price.toLocaleString('es-CO')} COP` : undefined);
     ...
     */
    static startKeepAlive(socket, userId) {
     ...
     */
    static async handleNewConversationalSystem(socket, userId, from, messageText, conversationId, message) {
        console.log(`[Baileys] 🚀 Usando NUEVO SISTEMA CONVERSACIONAL MODULAR`);
        try {
            // Importar el nuevo módulo conversacional
            const { procesarMensaje } = await Promise.resolve().then(() => __importStar(require('../conversational-module')));
            ...
            if (respuesta.texto) {
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, respuesta.texto, messageText.length);
                console.log(`[Baileys] ✅ Respuesta enviada con simulación humana`);
                // Guardar en BD
                await db_1.db.message.create({
                    data: {
            ...
            await human_typing_simulator_1.HumanTypingSimulator.quickHumanizedSend(socket, from, 'Disculpa, tuve un problema al procesar tu mensaje. ¿Podrías intentar de nuevo? 🙏');
        }
    }
    /**
     ...
     */
    static async detectAndHandlePayment(socket, userId, from, messageText, conversationId) {
        try {
...
¿Podrías decirme cuál producto quieres comprar?`;
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, noProductMessage, messageText.length);
            ...
            const paymentResult = await BotPaymentLinkGenerator.generatePaymentLinks(context.lastProductId, userId, 1 // cantidad
            );
            ...
            if (paymentResult.success && paymentResult.message) {
                ...
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, paymentResult.message, messageText.length);
                // Guardar en BD
...
Escríbeme para coordinar el pago 😊`;
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, from, fallbackMessage, messageText.length);
     ...
     * 🚀 NUEVO SISTEMA CONVERSACIONAL MODULAR
     */
    async handleNewConversationalSystem(socket, from, message) {
        ...
        try {
            ...
            const respuesta = await (0, conversational_module_1.procesarMensaje)(userId, messageText);
            // Enviar respuesta de texto con simulación humana
            if (respuesta.texto) {
     ...
     */
    static async sendHumanizedMessage(socket, chatId, message, userMessageLength = 50) {
        try {
            // Decidir si usar envío rápido o normal
            if (human_typing_simulator_1.HumanTypingSimulator.shouldUseQuickSend(message)) {
                await human_typing_simulator_1.HumanTypingSimulator.quickHumanizedSend(socket, chatId, message);
            }
            else {
                await human_typing_simulator_1.HumanTypingSimulator.humanizedSend(socket, chatId, message, userMessageLength);
            }
        }
        catch (error) {
...
exports.BaileysStableService = BaileysStableService;
BaileysStableService.sessions = new Map();
BaileysStableService.qrCallbacks = new Map();
BaileysStableService.reconnectTimers = new Map();
BaileysStableService.connectionLocks = new Map(); // 🔒 Bloqueo de conexiones con timestamp
BaileysStableService.keepAliveTimers = new Map(); // 💓 Keep-alive timers
BaileysStableService.hybridSystem = null;
BaileysStableService.conversationHistories = new Map();
// Logger silencioso
BaileysStableService.logger = (0, pino_1.default)({ level: 'silent' });
