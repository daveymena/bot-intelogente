...
exports.DropshippingSalesFlow = void 0;
const db_1 = require("./db");
class DropshippingSalesFlow {
    /**
     * Detectar si el mensaje es una consulta desde Facebook/redes sociales
     */
    static detectSocialMediaInquiry(message) {
     ...
     */
    static async startSalesFlow(from, message, userId) {
...
¿Deseas que te cuente los detalles o prefieres saber directamente el precio y forma de entrega?`;
     ...
     */
    static async continueSalesFlow(from, message, userId) {
...
¿Deseas que te deje el seguimiento por aquí cuando sea despachado?`;
            ...
            if (!data || !data.phone)
                missing.push('número de contacto');
...
¿Deseas que te muestre también la versión más grande del producto o prefieres cerrar con este modelo?`;
    ...
    static wantsPickup(message) {
        return /tienda|pasar|recoger|ver|persona|f[ií]sica/i.test(message);
    }
    static extractCustomerData(message, currentData) {
    ...
    static async createOrder(userId, customerPhone, product, customerData) {
        try {
...
exports.DropshippingSalesFlow = DropshippingSalesFlow;
DropshippingSalesFlow.flowStates = new Map();
