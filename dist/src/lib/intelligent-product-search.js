...
exports.intelligentProductSearch = intelligentProductSearch;
exports.generateProductResponse = generateProductResponse;
exports.hybridProductSearch = hybridProductSearch;
...
async function intelligentProductSearch(context) {
    ...
    const productMatch = await findProductWithAI(context.userMessage, allProducts, context.previousProducts || []);
    if (!productMatch) {
...
Para CONSULTA GENERAL:
{
  ...
  "shouldSendPhoto": false
    ...
    try {
        ...
        const { GroqAPIRotator } = await Promise.resolve().then(() => __importStar(require('./groq-api-rotator')));
        console.log('🤖 Llamando a Groq...');
        const response = await GroqAPIRotator.makeRequest([{ role: 'user', content: prompt }], {
            temperature: 0.3,
            maxTokens: 500
        });
 ...
 */
async function generateProductResponse(match, includePhoto = true) {
 ...
 */
async function hybridProductSearch(query, context) {
