...
exports.ConversationalSalesFlow = void 0;
const db_1 = require("./db");
class ConversationalSalesFlow {
    /**
     * Generar saludo inicial profesional
     */
    static generateGreeting(agentName = 'Laura') {
        const greetings = [
...
Soy ${agentName}, tu asesora personal. Estoy aquí para ayudarte a encontrar exactamente lo que necesitas. ¿Qué te interesa? 🛍️`
     ...
     */
    static generateProductPresentation(product) {
...
✨ Características principales:\n`;
     ...
     */
    static generatePhotoOffer(product) {
...
¿Qué te parece hasta ahora? 😊`;
     ...
     */
    static generateVariantOptions(product) {
     ...
     */
    static generateOrderDataRequest(product, variant) {
...
¿Podrías enviármelos para continuar? 📝`;
     ...
     * Confirmar pedido
     */
    static generateOrderConfirmation(product, customerData, variant) {
        const variantText = variant ? ` ${variant}` : '';
...
Si deseas, puedo agregarte a nuestra lista VIP 🔔 para enterarte de las próximas ofertas. ¿Te gustaría que te inscriba?`;
     ...
     * Cierre profesional
     */
    static generateProfessionalClosing() {
...
¡Te deseo un excelente día y disfruta tu nuevo producto! ✨`;
     ...
     */
    static handlePriceObjection(product) {
        const price = product.price.toLocaleString('es-CO');
...
¿Te gustaría que te muestre opciones de pago? O si prefieres, tengo productos similares en otros rangos de precio.`;
     ...
     */
    static async offerAlternatives(userId, currentProduct, maxBudget) {
...
¿Cuál sería tu presupuesto ideal? Así puedo buscar otras opciones que se ajusten mejor a lo que buscas.`;
     ...
     * Extraer características del producto
     */
    static extractFeatures(product) {
        const features = [];
     ...
     * Extraer variantes del producto
     */
    static extractVariants(product) {
        const variants = {};
        try {
            const tags = product.tags ? JSON.parse(product.tags) : [];
            // Buscar colores
            const colorKeywords = ['negro', 'blanco', 'gris', 'azul', 'rojo', 'verde', 'amarillo', 'rosa'];
            const colors = tags.filter((tag) => colorKeywords.some(color => tag.toLowerCase().includes(color)));
            if (colors.length > 0)
                variants.colors = colors;
            // Buscar tallas
            const sizeKeywords = ['S', 'M', 'L', 'XL', 'pequeño', 'mediano', 'grande'];
            const sizes = tags.filter((tag) => sizeKeywords.some(size => tag.toUpperCase().includes(size)));
            if (sizes.length > 0)
     ...
     */
    static getConversationStage(customerId) {
     ...
     */
    static updateConversationStage(customerId, updates) {
     ...
     */
    static clearConversation(customerId) {
        this.conversations.delete(customerId);
    }
}
exports.ConversationalSalesFlow = ConversationalSalesFlow;
ConversationalSalesFlow.conversations = new Map();
