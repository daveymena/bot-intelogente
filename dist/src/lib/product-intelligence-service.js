...
const fuzzy_match_service_1 = require("./fuzzy-match-service");
class ProductIntelligenceService {
    ...
    static async findProduct(query, userId) {
        try {
            ...
            const { TextNormalizer } = await Promise.resolve().then(() => __importStar(require('./text-normalizer')));
            ...
            const dictionary = fuzzy_match_service_1.FuzzyMatchService.getCommonProductTerms();
            const correctionResult = fuzzy_match_service_1.FuzzyMatchService.correctTypos(queryLower, dictionary, 0.7);
            ...
            const megapackIntent = fuzzy_match_service_1.FuzzyMatchService.detectMegapackIntent(correctedQuery);
            if (megapackIntent.isMegapackQuery) {
                ...
                if (megapackIntent.wantsAll) {
                    ...
                    const allMegapacks = await db_1.db.product.findMany({
                        where: {
                            userId,
                            status: 'AVAILABLE',
                            OR: [
                    ...
                    const pack40 = allMegapacks.find(p => p.name.includes('40') ||
                ...
                if (megapackIntent.specificNumber) {
                    ...
                    const specificPack = await db_1.db.product.findFirst({
                        where: {
                            userId,
                            status: 'AVAILABLE',
                            OR: [
                ...
                keywords.forEach(keyword => {
                    ...
                    if (nameLower.includes(keywordLower)) {
                        ...
                        for (const nameWord of nameWords) {
                            const similarity = fuzzy_match_service_1.FuzzyMatchService.calculateSimilarity(keywordLower, nameWord);
                            if (similarity >= 0.7) {
                    ...
                    else if (fuzzy_match_service_1.FuzzyMatchService.fuzzySearch(keywordLower, descLower, 0.7)) {
                        score += 2;
                    }
                    // Búsqueda en tags
                    ...
                    const nameWords = nameLower.split(/\s+/);
                    if (nameWords.some(word => word === keywordLower))
                        score += 10;
                });
     ...
     */
    static async generateResponse(product, intent, context) {
     ...
     */
    static generateInfoResponse(product, emoji, links, images) {
     ...
     */
    static generatePriceResponse(product, emoji, links) {
     ...
     */
    static generateLinkResponse(product, emoji, links) {
     ...
     */
    static generateBuyResponse(product, emoji, links) {
     ...
     */
    static generateAvailabilityResponse(product, emoji) {
        let response = `${emoji} ${product.name}\n\n`;
     ...
     */
    static generateGeneralResponse(product, emoji, links, images) {
        let response = `${emoji} **${product.name}**\n\n`;
     ...
     */
    static extractProductInfo(product) {
        return {
            name: product.name,
            price: product.price,
     ...
     */
    static generateStaticResponse(product, intent) {
        const emoji = this.getProductEmoji(product);
        const links = this.extractLinks(product);
        switch (intent.type) {
            case 'info':
    ...
    /**
         ...
         */
    static isGeneralProductQuery(query, keywords) {
        const queryLower = query.toLowerCase();
     ...
     */
    static async findProductsByCategory(query, userId, limit = 5) {
        try {
            const queryLower = query.toLowerCase();
     ...
     */
    static generateQualificationQuestions(query, products) {
        const queryLower = query.toLowerCase();
        // Detectar categoría
        let category = '';
        let questions = [];
