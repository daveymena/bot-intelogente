...
class UserLicenseService {
     ...
     */
    static async createSubscription(userId, email, plan = 'free', durationDays = 30) {
    ...
    static async checkUserSubscription(userId) {
        try {
            ...
            if (!user.subscriptionPlan || !user.subscriptionExpiresAt) {
                const subscription = await this.createSubscription(userId, user.email, 'free', 10 // 10 días de trial
                );
                return {
                    valid: true,
                    subscription,
            ...
            const subscription = {
                ...
                expiresAt,
                features: this.getFeaturesForPlan(user.subscriptionPlan),
                limits: this.getLimitsForPlan(user.subscriptionPlan),
            };
            return {
                valid: true,
                subscription,
     ...
     */
    static async upgradeSubscription(userId, newPlan, durationDays = 30) {
        try {
     ...
     */
    static async canUseFeature(userId, feature) {
    ...
    static async checkLimit(userId, limitType) {
        ...
        switch (limitType) {
            ...
            case 'products':
                ...
                current = await db_1.db.product.count({ where: { userId } });
                break;
            case 'conversations':
                limit = limits.maxConversations;
                current = await db_1.db.conversation.count({ where: { userId } });
                break;
        }
     ...
     */
    static getFeaturesForPlan(plan) {
        const features = {
