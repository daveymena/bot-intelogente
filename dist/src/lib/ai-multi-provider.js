...
const ai_model_selector_1 = require("./ai-model-selector");
class AIMultiProvider {
    ...
    static async tryOpenRouter(messages, options) {
    ...
    static async tryGroq(messages, options) {
        ...
        if (process.env.AI_AUTO_MODEL_DETECTION !== 'false') {
            try {
                const bestModel = await ai_model_selector_1.AIModelSelector.getBestAvailableModel();
                if (bestModel) {
                    model = bestModel;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Groq timeout')), timeout));
                ...
                if (errorMsg.includes('rate_limit_exceeded') || errorMsg.includes('429')) {
                    ...
                    ai_model_selector_1.AIModelSelector.markModelAsFailed(currentModel, 'Rate limit exceeded');
                    // Intentar obtener un modelo alternativo
                    const alternativeModel = await ai_model_selector_1.AIModelSelector.getAlternativeModel(currentModel);
                    if (alternativeModel) {
    ...
    static async tryLMStudio(messages, options) {
    ...
    static async tryOpenAI(messages, options) {
            ...
            const response = await fetch(`${ollamaUrl}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    options: {
                        ...
                        temperature: options.temperature || 0.7,
                        num_predict: options.max_tokens || 300, // Reducido para respuestas más rápidas
                        top_k: 40,
                        top_p: 0.9,
                        repeat_penalty: 1.1,
                        // Parámetros de rendimiento
                        num_ctx: 2048, // Contexto reducido para velocidad
                        num_batch: 512,
