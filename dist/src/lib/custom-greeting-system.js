...
exports.CustomGreetingSystem = void 0;
const db_1 = require("./db");
class CustomGreetingSystem {
    /**
     * Obtener el saludo personalizado del negocio
     */
    static async getCustomGreeting(userId) {
        ...
        const botSettings = await db_1.db.botSettings.findUnique({
            where: { userId }
        });
        // Obtener configuración de la tienda
        const storeSettings = await db_1.db.storeSettings.findUnique({
            where: { userId }
        });
...
Recuerda: Tu objetivo es VENDER usando información REAL, persuasión SUTIL y formato ATRACTIVO.`;
    ...
    static isGreeting(message) {
        ...
        const startsWithGreeting = greetings.some(greeting => messageLower.startsWith(greeting) && messageLower.length < 20);
    ...
    static getProductEmoji(product) {
        ...
        if (category === 'DIGITAL') {
            ...
            if (name.includes('software'))
                return '💻';
            if (name.includes('ebook') || name.includes('libro'))
                return '📖';
            return '💾';
        }
        ...
        if (name.includes('teclado') || name.includes('keyboard'))
            return '⌨️';
        if (name.includes('mouse') || name.includes('ratón'))
            return '🖱️';
...
exports.CustomGreetingSystem = CustomGreetingSystem;
