...
class PaymentConfigService {
    ...
    static async getConfig(userId) {
        let config = await db_1.db.paymentConfig.findUnique({
            where: { userId }
        });
        // Si no existe, crear configuración por defecto
        if (!config) {
            config = await db_1.db.paymentConfig.create({
                data: { userId }
            });
        }
        return config;
     ...
     */
    static async generatePaymentMessage(userId, productName, price, productId) {
     ...
     */
    static async hasPaymentMethods(userId) {
        const config = await this.getConfig(userId);
        return !!((config.mercadoPagoEnabled && config.mercadoPagoPublicKey) ||
