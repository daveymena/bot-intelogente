...
const ai_advanced_reasoning_1 = require("./ai-advanced-reasoning");
const conversation_budget_service_1 = require("./conversation-budget-service");
const product_context_manager_1 = require("./product-context-manager");
// import { TRAINING_SCENARIOS, BOT_RULES } from './sales-training-data' // Temporalmente desactivado
const intelligent_personality_service_1 = require("./intelligent-personality-service");
const professional_conversation_memory_1 = require("./professional-conversation-memory");
const megaflujos_service_1 = require("./megaflujos-service");
...
class AIService {
     ...
     */
    static async loadFullConversationHistory(userId, customerPhone) {
        try {
    ...
    static async generateResponse(userId, customerMessage, _customerPhone, conversationHistory = []) {
        try {
            ...
            const { HumanEscalationService } = await Promise.resolve().then(() => __importStar(require('./human-escalation-service')));
            ...
            if (escalation.needs) {
                ...
                await HumanEscalationService.notifyAdmin(userId, _customerPhone, customerName, escalation.category, customerMessage);
            ...
            professional_conversation_memory_1.ProfessionalConversationMemory.initMemory(conversationKey);
            await professional_conversation_memory_1.ProfessionalConversationMemory.hydrateFromStore(conversationKey);
            professional_conversation_memory_1.ProfessionalConversationMemory.incrementMessageCount(conversationKey);
            // 🚨 PRIORIDAD 1: Detectar limitación de presupuesto
            const budgetDetection = conversation_budget_service_1.ConversationBudgetService.detectBudgetConstraint(customerMessage);
            if (budgetDetection.hasBudget) {
                ...
                const productContext = product_context_manager_1.ProductContextManager.getContext(conversationKey);
                if (productContext) {
                    ...
                    if (budgetDetection.maxBudget) {
                        conversation_budget_service_1.ConversationBudgetService.setBudgetContext(conversationKey, budgetDetection.maxBudget, productContext.productId, productContext.productPrice, productContext.productName);
                        // 🧠 REGISTRAR EN MEMORIA PROFESIONAL
                        professional_conversation_memory_1.ProfessionalConversationMemory.setBudget(conversationKey, budgetDetection.maxBudget);
                        professional_conversation_memory_1.ProfessionalConversationMemory.addIntention(conversationKey, 'budget');
                    }
                    // Buscar alternativas más baratas
                    const cheaperAlternatives = await product_context_manager_1.ProductContextManager.findCheaperAlternatives(userId, productContext.productId, productContext.productPrice, budgetDetection.maxBudget, productContext.productCategory);
                    // Obtener producto actual completo
                    const currentProduct = await db_1.db.product.findUnique({
                        where: { id: productContext.productId }
                    });
                    if (currentProduct) {
                        const budgetResponse = conversation_budget_service_1.ConversationBudgetService.generateBudgetResponse(currentProduct, budgetDetection.maxBudget, cheaperAlternatives);
                        return {
            ...
            const { IntelligentPaymentDetector: PaymentDetectorEarly } = await Promise.resolve().then(() => __importStar(require('./intelligent-payment-detector')));
            ...
            if (isLikelyPaymentRequest) {
                ...
                const memory = professional_conversation_memory_1.ProfessionalConversationMemory.getMemory(conversationKey);
                ...
                if (memory && memory.currentProduct) {
                    ...
                    const productFromMemory = await db_1.db.product.findUnique({
                    ...
                    if (productFromMemory) {
                        ...
                        const paymentLinks = await BotPaymentLinkGenerator.generatePaymentLinks(productFromMemory.id, userId, 1);
                        if (paymentLinks.success) {
                            // 🧠 REGISTRAR INTENCIÓN DE PAGO
                            professional_conversation_memory_1.ProfessionalConversationMemory.addIntention(conversationKey, 'payment');
            ...
            const { QualificationFlowService } = await Promise.resolve().then(() => __importStar(require('./qualification-flow-service')));
            ...
            if (qualificationState && qualificationState.needsQualified) {
                ...
                const answerDetection = QualificationFlowService.isQualificationAnswer(customerMessage, qualificationState.category);
                if (answerDetection.isAnswer && answerDetection.intent) {
                    ...
                    const filteredProducts = await QualificationFlowService.filterProductsByQualification(userId, qualificationState.category, answerDetection.intent);
                    if (filteredProducts.length > 0) {
                        // Generar respuesta con productos filtrados
                        const intro = QualificationFlowService.generateFilteredResponse(filteredProducts, qualificationState.category, answerDetection.intent);
                        // Formatear lista de productos
                        const { ProductListFormatter } = await Promise.resolve().then(() => __importStar(require('./product-list-formatter')));
                        const formattedList = ProductListFormatter.formatProductList(filteredProducts, customerMessage);
                        ...
                        if (filteredProducts.length > 0) {
                            professional_conversation_memory_1.ProfessionalConversationMemory.setCurrentProduct(conversationKey, filteredProducts[0].id, filteredProducts[0].name, filteredProducts[0].price, filteredProducts[0].category);
                        }
                        return {
                            message: `${intro}\n\n${formattedList}`,
                            confidence: 0.95,
            ...
            if (generalQuery.isGeneral && generalQuery.category) {
                ...
                const qualificationQuestion = QualificationFlowService.generateQualificationQuestion(generalQuery.category);
                // Guardar estado de calificación
                QualificationFlowService.setQualificationState(conversationKey, generalQuery.category, true);
                return {
            ...
            let productIntent = product_intelligence_service_1.ProductIntelligenceService.detectIntent(customerMessage);
            if (productIntent.confidence > 0.7) {
                ...
                professional_conversation_memory_1.ProfessionalConversationMemory.addIntention(conversationKey, productIntent.type);
                // Verificar si hay contexto de producto bloqueado
                const existingContext = product_context_manager_1.ProductContextManager.getContext(conversationKey);
                ...
                if (existingContext && existingContext.isLocked && !isExplicitChange) {
                    ...
                    if (product) {
                        ...
                        const focusedResponse = product_context_manager_1.ProductContextManager.generateFocusedResponse(product, customerMessage, productIntent.type);
                        if (focusedResponse) {
                            product_context_manager_1.ProductContextManager.incrementMessageCount(conversationKey);
                            return {
                ...
                let product = await product_intelligence_service_1.ProductIntelligenceService.findProduct(customerMessage, userId);
                ...
                if (product) {
                    ...
                    if (!context || context.lastProductId !== product.id) {
                        ...
                        product_context_manager_1.ProductContextManager.setContext(conversationKey, product.id, product.name, product.price, product.category, true // Bloquear contexto para mantener el foco
                        );
                        // 🧠 GUARDAR EN MEMORIA PROFESIONAL
                        professional_conversation_memory_1.ProfessionalConversationMemory.setCurrentProduct(conversationKey, product.id, product.name, product.price, product.category);
                    }
                }
                        ...
                        for (let i = conversationHistory.length - 1; i >= Math.max(0, conversationHistory.length - 6); i--) {
                            ...
                            if (historicalMessage.role === 'user') {
                                ...
                                if (foundProduct) {
                                    ...
                                    // 🧠 GUARDAR EN MEMORIA PROFESIONAL
                                    professional_conversation_memory_1.ProfessionalConversationMemory.setCurrentProduct(conversationKey, foundProduct.id, foundProduct.name, foundProduct.price, foundProduct.category);
                ...
                const isGeneral = product_intelligence_service_1.ProductIntelligenceService.isGeneralProductQuery(customerMessage, keywords);
                if (isGeneral && !product) {
                    ...
                    const categoryProducts = await product_intelligence_service_1.ProductIntelligenceService.findProductsByCategory(customerMessage, userId, 5);
                    if (categoryProducts.length > 0) {
                        ...
                        const { ProductListFormatter } = await Promise.resolve().then(() => __importStar(require('./product-list-formatter')));
                        const formattedList = ProductListFormatter.formatProductList(categoryProducts, customerMessage);
                        return {
                ...
                if (product) {
                    ...
                    const productInfo = product_intelligence_service_1.ProductIntelligenceService.extractProductInfo(product);
                    ...
                    const aiResponse = await this.generateProductResponse(customerMessage, product, productInfo, productIntent, fullHistory.length > 0 ? fullHistory : conversationHistory, conversationKey, userId);
...
📞 WhatsApp: +57 304 274 8687`,
            ...
            if (relevantProducts.length > 0) {
                ...
                conversation_context_service_1.ConversationContextService.setProductContext(conversationKey, topProduct.id, topProduct.name);
                product_context_manager_1.ProductContextManager.setContext(conversationKey, topProduct.id, topProduct.name, topProduct.price, topProduct.category, true // Bloquear contexto
                );
                // 🧠 GUARDAR EN MEMORIA PROFESIONAL
                professional_conversation_memory_1.ProfessionalConversationMemory.setCurrentProduct(conversationKey, topProduct.id, topProduct.name, topProduct.price, topProduct.category);
                console.log(`[AI] 🧠 Producto guardado en memoria profesional: ${topProduct.name}`);
            }
            // Obtener configuración de prompts personalizados
            const customPrompts = await db_1.db.aIPrompt.findMany({
                where: {
                    userId,
                    isActive: true
                }
            ...
            let systemPrompt = await intelligent_personality_service_1.IntelligentPersonalityService.buildSystemPrompt(userId, businessContext, productsInfo);
            // 📚 AGREGAR CONTEXTO DE MEGAFLUJOS
            const megaflujoContexto = megaflujos_service_1.MegaflujoService.obtenerContextoParaPrompt(customerMessage);
            ...
            if (USE_ADVANCED_REASONING) {
                ...
                const aiResponse = await ai_advanced_reasoning_1.AIAdvancedReasoning.generateConversationalResponse(customerMessage, fullHistory, systemPrompt);
    ...
    static findRelevantProducts(message, allProducts) {
        ...
        const relevant = allProducts.filter(p => {
            ...
            const hasNameMatch = nameWords.some(word => word.length > 3 && messageWords.some(mw => mw.includes(word) || word.includes(mw)));
            // Si el mensaje menciona palabras de la descripción
            const hasDescMatch = descLower.split(' ').some(word => word.length > 4 && messageLower.includes(word));
            // Buscar en tags
            let hasTagMatch = false;
            try {
                const tags = p.tags ? JSON.parse(p.tags) : [];
                hasTagMatch = tags.some((tag) => messageLower.includes(tag.toLowerCase()));
            }
            catch (e) {
        ...
        if (messageLower.includes('laptop') || messageLower.includes('portatil') || messageLower.includes('computador')) {
            return allProducts.filter(p => p.name.toLowerCase().includes('laptop') ||
                p.name.toLowerCase().includes('vivobook') ||
                p.name.toLowerCase().includes('macbook')).slice(0, 5);
        }
        if (messageLower.includes('curso') || messageLower.includes('mega pack')) {
            return allProducts.filter(p => p.name.toLowerCase().includes('curso') ||
                p.name.toLowerCase().includes('mega pack')).slice(0, 5);
        }
        if (messageLower.includes('moto')) {
            return allProducts.filter(p => p.name.toLowerCase().includes('moto') ||
                p.name.toLowerCase().includes('pulsar') ||
                p.name.toLowerCase().includes('bajaj')).slice(0, 5);
        }
        // Si no hay coincidencias, retornar productos destacados
        return allProducts.slice(0, 5);
    }
    // Construir contexto del negocio
    static buildBusinessContext(user, customPrompts) {
    ...
    static buildProductsInfo(products) {
        ...
        return products.map(p => {
            ...
            try {
                ...
                const paypalTag = tags.find((t) => t.startsWith('paypal:'));
                const nequiTag = tags.find((t) => t.startsWith('nequi:'));
                const paycoTag = tags.find((t) => t.startsWith('payco:'));
                const contactoTag = tags.find((t) => t.startsWith('contacto:'));
                // Links directos (sin prefijo)
                const enlaces = tags.filter((t) => t.startsWith('http'));
                ...
                const infoLink = enlaces.find((link) => link.includes('landein') || link.includes('page') || link.includes('info') || link.includes('vercel'));
                if (infoLink) {
    ...
    static buildSystemPrompt(businessContext, products, customPersonality = null) {
...
Responde SIEMPRE en español, de forma profesional y honesta.`;
    ...
    static async generateProductResponse(customerMessage, product, productInfo, intent, conversationHistory, conversationKey, userId) {
        try {
            // 🌐 VERIFICAR SI NECESITA INFORMACIÓN EXTERNA
            const { ExternalKnowledgeService } = await Promise.resolve().then(() => __importStar(require('./external-knowledge-service')));
...
`;
            ...
            if (externalInfo && externalInfo.found) {
                ...
                if (externalInfo.features && externalInfo.features.length > 0) {
                    ...
                    externalInfo.features.forEach((feature) => {
...
`;
            ...
            const memoryContext = professional_conversation_memory_1.ProfessionalConversationMemory.generateContextSummary(conversationKey);
            // 🎭 CARGAR PERSONALIDAD PERSONALIZADA (si existe)
            const { AIPersonalityLoader } = await Promise.resolve().then(() => __importStar(require('./ai-personality-loader')));
...
Responde al cliente de forma natural, profesional y orientada a la venta:`;
            ...
            if (messageCount > 2)
                maxTokens = 250; // Ya conoce el producto: más conciso
            if (messageCount > 5)
                maxTokens = 150; // Ya sabe todo: ultra conciso
            ...
            return product_intelligence_service_1.ProductIntelligenceService.generateStaticResponse(product, intent);
        }
    }
    // Verificar si debe responder automáticamente
    static shouldAutoRespond(message) {
     ...
     */
    static detectExplicitProductChange(message, currentContext) {
        const normalized = message.toLowerCase().trim();
        ...
        if (currentContext && currentContext.productName) {
            ...
            const mentionsCurrentProduct = currentProductWords.some(word => word.length > 3 && messageWords.includes(word));
