...
class SearchAgent extends base_agent_1.BaseAgent {
    constructor() {
        super('SearchAgent');
    }
    /**
    ...
    async handleLocally(message, memory) {
        ...
        const { SharedMemoryService } = await Promise.resolve().then(() => __importStar(require('./shared-memory')));
     ...
     */
    findProductInList(message, products) {
        const cleanMsg = this.cleanMessage(message);
     ...
     */
    calculateProductScore(product, fullQuery, keywords) {
        ...
        if (specificKeywords.length > 0) {
            const hasAnySpecificKeyword = specificKeywords.some(k => name.includes(k) || description.includes(k) || tags.includes(k));
    ...
    extractSpecificKeywords(query) {
        ...
        if (/\bui\b/.test(query) || /\bux\b/.test(query)) {
            if (/\bui\b/.test(query))
                specificWords.push('ui');
            if (/\bux\b/.test(query))
                specificWords.push('ux');
        }
    ...
    mapProduct(p) {
        ...
        if (p.images) {
            if (typeof p.images === 'string') {
                ...
                if (p.images.startsWith('http')) {
                    ...
                    try {
                        ...
                        images = Array.isArray(parsed)
                            ? parsed.filter((img) => typeof img === 'string' && img.startsWith('http'))
                            : [];
                    }
                    catch (e) {
                ...
                images = p.images.filter((img) => typeof img === 'string' && img.startsWith('http'));
            }
        }
        return {
            id: p.id,
