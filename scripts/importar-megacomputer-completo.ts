...
async function importar() {
  console.log('📦 IMPORTANDO PRODUCTOS DE MEGACOMPUTER');
  console.log('='.repeat(70));

  const jsonPath = path.join(process.cwd(), 'productos-megacomputer-completo.json');
  
  if (!fs.existsSync(jsonPath)) {
    console.log('❌ No se encontró productos-megacomputer-completo.json');
    return;
  }
  ...
  for (const prod of productos) {
    try {
      ...
      const data = {
        ...
        images: JSON.stringify(prod.images || []), // Convertir array a JSON string
      ...
      if (fotos.length > 0) {
        console.log(`   📸 Fotos: ${fotos.length} - ${fotos[0]}`);
      } else {
