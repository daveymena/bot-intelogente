...
export class ProductTagService {
  ...
  static validateTagConfig(config: ProductTagConfig): {
    ...
    if (config.tags.length < 3) {
      warnings.push('Se recomiendan al menos 3 tags para mejor búsqueda')
      suggestions.push('Agrega más palabras clave relacionadas con tu producto')
    }

    if (config.tags.length > 15) {
      warnings.push('Demasiados tags pueden reducir la relevancia')
      suggestions.push('Considera reducir a los 10-12 tags más importantes')
    }

    if (!config.category) {
      warnings.push('No se ha definido una categoría')
      suggestions.push('Selecciona una categoría para mejorar la organización')
    }
  ...
  private static detectProductType(text: string): string[] {
    ...
    const typePatterns = {
      'curso': /curso|capacitación|formación|entrenamiento/i,
      'digital': /digital|online|virtual|descarga/i,
      'físico': /físico|material|envío|entrega/i,
      'software': /software|programa|aplicación|app/i,
      'ebook': /ebook|libro|guía|manual/i,
      'video': /video|tutorial|clase|lección/i,
      'audio': /audio|podcast|música|sonido/i
    }
  ...
  private static detectCategory(productName: string): string {
    ...
    const categoryPatterns: Record<string, RegExp> = {
      'cursos': /curso|capacitación|formación/i,
      'musica': /música|piano|guitarra|instrumento/i,
      'idiomas': /inglés|español|idioma|language/i,
      'diseno': /diseño|photoshop|illustrator/i,
      'programacion': /programación|código|desarrollo|javascript|python/i,
      'marketing': /marketing|ventas|publicidad/i,
      'fotografia': /fotografía|foto|cámara/i,
      'video': /video|edición|premiere/i,
      'negocios': /negocio|empresa|emprendimiento/i,
      'tecnologia': /tecnología|tech|software/i
    }
