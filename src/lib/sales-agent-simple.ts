...
export function calculateTypingDelay(responseLength: number): number {
  ...
  return Math.min(calculated, 1500) // Máximo 1.5 segundos
}
...
async function askGroq(prompt: string, context: string = '', products: any[] = []): Promise<string | null> {
  try {
    ...
    const digitales = products.filter(p => p.category === 'DIGITAL' || p.name.toLowerCase().includes('mega') || p.name.toLowerCase().includes('curso') || p.name.toLowerCase().includes('programación') || p.name.toLowerCase().includes('pack'))
...
- "programasion", "programacion" ? buscar "Programación", "Desarrollo", "Software" o "Pack"
...
- "curso de programacion" ? buscar producto con "Programación", "Desarrollo" o "Mega Pack"
...
Responde SOLO con el número del producto (1, 2, 3...) o "0" si no hay coincidencia.`
    ...
    console.log(`?? Error en búsqueda IA: ${error.message}`)
    return { product: null, response: null }
  }
}
...
export class SalesAgentSimple {
  ...
  async processMessage(message: string, userPhone: string, context?: any): Promise<ProcessedResponse> {
    try {
      ...
      // ?? OPTIMIZACIÓN: Prioridad absoluta a Regex para velocidad
      // Si detectamos intención clara por reglas locales, SALTAMOS la IA
      const regexIntent = this.detectIntent(message)
      let aiDecision: AIDecision

      if (regexIntent !== 'general_inquiry') {
        console.log(`?? Regex detectó: ${regexIntent} (Saltando análisis AI)`)
        aiDecision = {
          action: regexIntent,
        ...
        // ?? IA RAZONAMIENTO: Solo si regex no detectó nada claro
        try {
  ...
  private detectIntent(message: string): string {
    ...
    //  DESPEDIDA CON INTERÉS FUTURO - Patrones con "perfecto/ok/vale" + indicador de futuro
    // IMPORTANTE: Detectar ANTES de confirmationPatterns para evitar falsos positivos
    if (/^(perfecto|ok|vale|bueno|listo|dale)[,.\s]+(más tarde|mas tarde|luego|después|despues|te (aviso|digo|confirmo|escribo))/i.test(msg)) {
  ...
  private async generatePaymentResponse(product: any): Promise<string> {
    ...
    if (isPhysical) {
      ...
      response += `2?? *Contraentrega* (Envío a domicilio)\n`
  ...
  private handleDigitalProductFAQ(message: string, product: any): string {
    ...
    if (/(cómo\s*(lo\s*)?(recibo|obtengo|accedo)|por\s*dónde\s*(lo\s*)?(envían|mandan|recibo)|google\s*drive|link\s*de\s*(descarga|acceso)|entrega\s*inmediata|acceso\s*inmediato)/i.test(msg)) {
      return `¡Entrega INMEDIATA! ?\n\n` +
        ...
        `3?? Te envío el link de Google Drive al instante\n\n` +
...
RESPUESTA (en español):`;
    ...
    try {
      ...
      console.log('?? Ollama falló en generateAIAnswer, usando Groq...');
    }
