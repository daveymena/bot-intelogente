...
export const TOOLS: any = {
    list_products_by_category: {
        ...
        description: 'Lista MÚLTIPLES productos cuando el usuario busca una categoría general (ej: "computadores", "cursos", "laptops"). Muestra 3-5 opciones.',
    ...
    get_product_with_payment: {
        ...
        description: 'Busca UN producto ESPECÍFICO cuando el usuario menciona un nombre exacto (ej: "curso de piano", "laptop asus vivobook").',
        execute: async (params: any, context: any) => {
            try {
...
class OpenClawOrchestrator {
    ...
    async processMessage(messageText: string, from: string, context: any) {
        ...
        try {
            const Fuse = (await import('fuse.js')).default;
            const fuse = new Fuse(context.products, { threshold: 0.6, keys: ['name', 'tags', 'description'] });
            const hints = fuse.search(messageText).slice(0, 5);
            if (hints.length > 0) {
                catalogHints = hints.map(h => `ID: ${h.item.id} | NOMBRE: ${h.item.name} | TIPO: ${h.item.tipo_producto} | ENTREGA: ${h.item.tipo_entrega}`).join('\n');
            }
        } catch (e) { /* silent */ }
...
### 🏷️ CATÁLOGO HINTS (Productos reales que puedes nombrar):
${catalogHints}
...
3. **Búsqueda General**: Si el usuario es vago ("busco laptop", "qué tienes?"), usa 'list_products_by_category'.
4. **Pagos**: Si pregunta cómo pagar o pide cuentas, usa 'get_payment_info'.
5. **Chat**: Solo responde sin herramientas si es un saludo ("hola", "buen día") o despedida.

Responde SOLO JSON:
{ 
  "reasoning": "Por qué elegí esta herramienta (o por qué ninguna)",
  "toolToUse": "nombre_o_null", 
  "toolParams": { "productId": "nombre del producto encontrado en hints", "searchTerm": "término de búsqueda" } 
}`;
...
`;
        ...
        if (isProductList) {
            systemPrompt += `
### asesorando_producto (MODO ASESOR):
MUESTRA LA LISTA SIGUIENDO ESTE FORMATO EXACTO. 

PRODUCTOS:
${toolData.products.map((p: any, i: number) => `${i+1}. ${p.name} - $${p.price}`).join('\n')}

FORMATO OBLIGATORIO (NO AÑADAS TEXTO EXTRA):
¡[Saludo corto]! Te ayudo a elegir de nuestro catálogo:

━━━━━━━━━━━━━━━━━━
${toolData.products.slice(0, 3).map((p: any, i: number) => `${i+1}️⃣ *${p.name}* - $${p.price}`).join('\n')}
━━━━━━━━━━━━━━━━━━
