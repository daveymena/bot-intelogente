...
export class UniversalSalesFlow {
  ...
  private static async processStep(
    ...
    if (state.step === 'price' || state.step === 'capture_data') {
      ...
      if (this.wantsShipping(normalized)) {
        ...
        if (config.requireName) response += `📍 *Nombre completo*\n`
        if (config.requirePhone) response += `📞 *Teléfono de contacto*\n`
        if (config.requireAddress) response += `🏠 *Dirección completa*\n`
        if (config.requireCity) response += `🏙️ *Ciudad*\n`
        if (config.showColors && product) response += `🎨 *Color preferido*\n`
  ...
  private static getMissingData(data: any, config: any): string[] {
    ...
    if (config.requirePhone && !data.phone) missing.push('teléfono')
    if (config.requireEmail && !data.email) missing.push('email')
    if (config.requireAddress && !data.address) missing.push('dirección')
